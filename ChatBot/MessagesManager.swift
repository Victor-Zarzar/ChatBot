...
//  ChatBot
//
//  Created by Victor Zarzar on 11/02/23.
//
...
class MessagesManager: ObservableObject {
    ...
    func getMessages() {
        db.collection("messages").addSnapshotListener { QuerySnapshot, error in
            ...
            // Mapping throungh the documents
            self.messages = documents.compactMap { document -> Message? in
                do {
                    
                    // Converting each document into the Message model
                    // Note that data(as:) is a function available only inf FirebaseFirestoreSwift package - remenber to import it at the top
